...
5. **Lambda code generating localhost URLs** (invitation links, email links, redirects) ⭐
6. **CDK environment variables still set to localhost** (FRONTEND_URL, CALLBACK_URL) ⭐
7. **Vercel deploying as Preview instead of Production**
8. **API Gateway CORS not configured for production origin**
9. **Missing endpoints that localhost proxy was masking**
10. **Cookie/session issues due to cross-origin**
...
**Check for hardcoded URLs in Lambda code (invitation links, emails, redirects):** ⭐
```bash
# Find any Lambda code that generates URLs users will see
grep -rn "localhost\|127.0.0.1\|signup?token\|invitation.*link\|invite.*url\|redirect\|callback" \
  lambda/ --include="*.js" | grep -v node_modules

# Check CDK environment variables for localhost
grep -rn "FRONTEND_URL\|CALLBACK\|REDIRECT\|localhost" lib/ --include="*.ts"

# Check Lambda env vars in AWS
aws lambda list-functions --region YOUR_REGION \
  --query "Functions[?contains(FunctionName, 'YourProject')].FunctionName" --output text | \
  while read fname; do
    VARS=$(aws lambda get-function-configuration --function-name "$fname" --region YOUR_REGION \
      --query "Environment.Variables" --output text 2>/dev/null | grep -i "localhost")
    if [ ! -z "$VARS" ]; then
      echo "❌ $fname has localhost in env vars: $VARS"
    fi
  done
```
...
```bash
# Check for hardcoded localhost URLs IN ALL CODE (frontend, backend, CDK)
grep -rn "localhost\|127.0.0.1" frontend/ lambda/ lib/ --include="*.ts" --include="*.tsx" --include="*.js" \
  | grep -v node_modules | grep -v ".next" | grep -v ".env"
# Should find NOTHING in production paths (all URLs should use environment variables)
# Common offenders:
#   - Lambda invitation/email link generators (e.g., signup?token=...)
#   - CDK environment variables (FRONTEND_URL: 'http://localhost:3000')
#   - API base URLs in frontend code
#   - Redirect URLs after auth flows
...
- [ ] No hardcoded localhost URLs in Lambda code (invitation links, emails, redirects) ⭐
- [ ] CDK environment variables use production URLs (FRONTEND_URL, etc.) ⭐
